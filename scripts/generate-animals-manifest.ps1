# generate-animals-manifest.ps1
# Scans images/animals/ directory and creates a manifest.json listing all image files

param(
  [string]$TargetDir = "images/animals",
  [string]$OutputFile = "images/animals/manifest.json"
)

# Validate target directory exists
if (-not (Test-Path $TargetDir -PathType Container)) {
  Write-Error "Target directory does not exist: $TargetDir"
  exit 1
}

# Get all image files in the target directory (not in subdirectories), prioritizing .png files
$files = Get-ChildItem -Path $TargetDir -File | Where-Object { $_.Extension -match '(?i)^(\.png)$' } | ForEach-Object { "$TargetDir/$($_.Name)" }

# Sort files alphabetically
$files = $files | Sort-Object

# Convert to JSON array (ensure array shape even for single item)
$json = @($files) | ConvertTo-Json -Depth 1

# Create output directory if it doesn't exist
$outputDir = Split-Path -Parent $OutputFile
if (-not (Test-Path $outputDir -PathType Container)) {
  New-Item -ItemType Directory -Path $outputDir -Force | Out-Null
}

# Write to file
$json | Out-File -FilePath $OutputFile -Encoding UTF8

Write-Host "Generated manifest: $OutputFile"
Write-Host "Total files: $($files.Count)"
exit 0
